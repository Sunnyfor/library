...
abstract class BaseReadAloudService : BaseService(),
    ...
    fun newReadAloud(play: Boolean, pageIndex: Int, startPos: Int) {
        execute(executeContext = IO) {
            Log.i("ZyLog","处理朗读逻辑 newReadAloud：pageIndex${pageIndex}  ，startPos：$pageIndex")
            ...
            readAloudNumber = 0
            contentList = splitAndKeepEndSymbols(textChapter.getPage(pageIndex)?.text?:"")
            var pos = startPos
    ...
    private fun prevP() {
        if (nowSpeak > 0) {
            ...
            textChapter?.let {
                ...
                if (readAloudNumber < 0) {
                    pageIndex--
                    ReadBook.moveToPrevPage()
                    newReadAloud(true,pageIndex,0)
                }
            }
    ...
    fun splitAndKeepEndSymbols(input: String): List<String> {
        val regex = Regex("[${AppPattern.passagesRegex}]") // 创建正则表达式实例
        val resultList = mutableListOf<String>()
        var lastEnd = 0

        regex.findAll(input).forEach { matchResult ->
            val matchStart = matchResult.range.first
            // 添加从上次结束位置到本次匹配前的文本，以及匹配到的分隔符
            val value = (input.substring(lastEnd until matchStart) + matchResult.value).split("\n").filter { it.isNotEmpty() }
            resultList.addAll(value)
            lastEnd = matchStart + matchResult.value.length
        }

        // 添加最后剩余的部分，如果没有分隔符则直接添加整个剩余部分
        if (lastEnd < input.length) {
            val value = input.substring(lastEnd).split("\n").filter { it.isNotEmpty() }
            resultList.addAll(value)
        }
