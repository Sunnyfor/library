    ...
    private inner class TTSUtteranceListener : UtteranceProgressListener() {
        ...
        override fun onStart(s: String) {
            ...
            textChapter?.let {
                ...
                LogUtils.d("ZYDEBUG","开始：readAloudNumber:${ readAloudNumber}")
        ...
        override fun onDone(s: String) {
            ...
            //跳过全标点段落
            do {
                readAloudNumber += contentList[nowSpeak].length   - paragraphStartPos
                LogUtils.d("ZYDEBUG","当前进度：$$readAloudNumber  content:${contentList[nowSpeak]} “paragraphStartPos：$paragraphStartPos")
//                LogUtils.d("ZYDEBUG","下一句开始：${contentList.toString()[readAloudNumber]}")
                paragraphStartPos = 0
                nowSpeak++
                if (nowSpeak >= contentList.size) {
                    if (pageIndex >= (textChapter?.pageSize?:0)-1){
                        nextChapter()
                    }else{
                        pageIndex++
