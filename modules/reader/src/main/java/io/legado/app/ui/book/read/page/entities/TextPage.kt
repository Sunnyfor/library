...
data class TextPage(
    ...
    fun upPageAloudSpan(aloudSpanStart: Int) {
        ...
        var currentOffset = 0 // 初始化当前偏移量
        // 遍历文本行
        for (textLine in textLines) {
            textLine.columns.forEachIndexed { index, baseColumn ->
                currentOffset++
                if (currentOffset >= aloudSpanStart) {
                    if (baseColumn is TextColumn) {
                        baseColumn.selected = true
                    }
                    if (AppPattern.passagesRegex.contains(baseColumn.toString()) || (index == textLine.columns.size - 1 && textLine.isParagraphEnd)) {
                        return // 遇到停止条件后直接退出
                    }
                }
